...
# need to remove first 3 lines and all instances of "*"


```{r}
TE <- read.table("/Users/hellebaalsrud/Documents/Projects/Mucormycota/Repeats/gzMucCirc2.fa.out", header = T, na.strings = ("."))
```

```{r pressure, echo=FALSE}
colnames(TE) <- c("SW_score","percdiv","perc.del","perc.ins","query.seq","query.start","query.end","left","dunno","repeat_type","repeat.class.family","repeat.startl","repeat.end","left2","ID")
```
...
g <- ggplot(data=TE2, aes(x=repeat.class.family, fill=repeat.class)) +
# Number of cars in each class:
geom_bar() + theme_light()+ theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + geom_text(aes(label = ..count..), stat = "count", hjust=2, angle = 90, colour="white")  + scale_y_log10() +labs(title = 'B.)') 
g
